...
class DataProcessor:
    ...
    def generate_target_pairs(self, target):
        self.graph_pair_data = []
        for graph_b in self.graph_data:
            if(graph_b != target):
                self.graph_pair_data.append((target, graph_b))
    ...
    def add_pairs_from_location(self, location):
        self.graph_pair_data = []
        for idx_graph_a, idx_graph_b in itertools.combinations(range(len(self.graph_data)), 2):
            self.graph_pair_data.append((self.graph_data[idx_graph_a], self.graph_data[idx_graph_b]))
        #TODO: different sampling technique?
...
class DFGGenerator:
    ...
    def process(self, verilog_file):
        dataflow_analyzer = VerilogDataflowAnalyzer(verilog_file, "top_module")
        ...
        dfg_graph_generator = VerilogGraphGenerator("top_module", terms, binddict, resolved_terms, 
...
class CFGGenerator:
    ...
    def process(self, verilog_file): 
        ...
        dataflow_analyzer = VerilogDataflowAnalyzer(self.verilog_file, "top_module")
        dataflow_analyzer.generate()
        binddict = dataflow_analyzer.getBinddict()
        terms = dataflow_analyzer.getTerms()
        ...
        resolved_binddict = dataflow_optimizer.getResolvedBinddict()
        constlist = dataflow_optimizer.getConstlist()
        cfg_graph_generator = VerilogControlflowAnalyzer("top_module", terms, binddict, resolved_terms, resolved_binddict, constlist, fsm_vars)
...
class HW2GRAPH:
    ...
    def rename_topModule(self, hw_path):
        ...
        for line in lines:
            words = line.split()
            for word in words:
                if word in modules_dic.keys() and modules_dic[word] != 1:
                    modules_dic[word] += 1

        for m in modules_dic:
            #print(modules_dic[m])
            if modules_dic[m] == 1:
                #print("Top module found: " + str(m))
                top_module = m
                break
    ...
    def preprocess(self, verilog_dir):
        flattened_hw_path = str(verilog_dir) + "/topModule.v"
