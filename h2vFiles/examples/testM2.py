...
#data_proc.Vasudev_generate_pairs('c432_orig')
#data_proc.Vasudev_generate_pairs('c6288')
all_pairs = data_proc.get_pairs()
for pair_idx, pair in enumerate(all_pairs):
    graph_a, graph_b = pair
    #print(graph_b.hw_name)
...
test_loader  = DataLoader(all_pairs, shuffle=False, batch_size=1)#cfg.batch_size)
'''model configuration'''
model = GRAPH2VEC(cfg)
if cfg.model_path != "":
...
trainer.model = model
print("Loss")
test_loss, test_labels, similarity_scores, test_preds = trainer.inference(test_loader)
#print("Labels")
#print(test_labels)
#print("Predictions")
#print(test_preds)
...
for i in range(len(test_preds_list)):
    if test_preds_list[i] == True:
        print(all_pairs[i][1].hw_name)
'''
trainer.metric_calc(test_loss,  test_labels,  test_preds,  header="test ")
