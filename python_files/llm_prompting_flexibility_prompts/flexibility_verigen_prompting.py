...
def write_results(prompt, result_text, iteration, sample, module_name):
    ...
    filepath = "flexibility_verigen6b_dump2/flex_07/" + iteration +"_" + sample + "_" + module_name + ".v"
...
for iteration, dir in enumerate(pair_dirs):
    ...
    for sample in range(10):
        ...
        result_file = write_results(prompt_text, generation, str(index), str(sample), module_name)
        testbench_path = tb_dir + "/" + module_name + "/" + module_name + "_0_tb.v"
        if os.path.exists(testbench_path):
            output_file = "flexibility_verigen6b_dump/flex_07/" + module_name + "_compile" + str(index) + "_"+ str(sample)
